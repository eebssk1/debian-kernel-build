...
if [ x$2 = xgcc ]; then
curl -L https://github.com/eebssk1/aio_tc_build/releases/download/604ce955/x86_64-linux-gnu-native.tb2 | tar --bz2 -xf -
...
if [ x$2 = xgcc ]; then
KCFLAGS="-O3 -fgraphite -fgraphite-identity -fipa-pta -flimit-function-alignment -flive-range-shrinkage -fsched-pressure -fsched-spec-load -fsched-stalled-insns=5 -fsched-stalled-insns-dep=8 -fschedule-insns -ftree-cselim -ftree-lrs -malign-data=cacheline -mrelax-cmpxchg-loop @$PWD/../gt.txt"
else
